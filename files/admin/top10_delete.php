...
$stmt = mysqli_prepare($myConnection, "SELECT id, top10_name FROM t_top10 WHERE id = ?");
...
$display_name = $entry['top10_name'] === '<hr>' ? 'this divider line' : $entry['top10_name'];

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_delete'])) {
    $stmt = mysqli_prepare($myConnection, "DELETE FROM t_top10 WHERE id = ?");
    mysqli_stmt_bind_param($stmt, 'i', $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    log_audit($myConnection, 'top10', $id, 'delete', $entry['top10_name'], $_SESSION['user_id']);
...
<html>
<head>
<title>AmigaSource.com - Delete Top 10 Entry</title>
				...
				<table width="100%" cellpadding="1" cellspacing="1" class="bg-white" bgcolor="<?php echo bg_hex('white'); ?>">
					<tr>
						<td align="center" class="bg-red" bgcolor="<?php echo bg_hex('red'); ?>">
							<font class="txt-4-white" face="Verdana, sans-serif" size="4" color="<?php echo txt_hex('white'); ?>"><b>DELETE TOP 10 ENTRY</b></font>
						</td>
					</tr>
					<tr>
						<td class="bg-whitesmoke" bgcolor="<?php echo bg_hex('whitesmoke'); ?>" style="padding:16px;">
							<font class="txt-2-black" face="Verdana, sans-serif" size="2" color="<?php echo txt_hex('black'); ?>">
								Are you sure you want to delete <b><?php echo htmlspecialchars($display_name); ?></b>?
							</font>
							<br><br>
							<center>
								<form method="post" action="top10_delete.php" style="display:inline;">
