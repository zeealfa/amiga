...
$stmt = mysqli_prepare($myConnection, "SELECT id, cfund_name FROM t_cfund WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_delete'])) {
    $stmt = mysqli_prepare($myConnection, "DELETE FROM t_cfund WHERE id = ?");
    mysqli_stmt_bind_param($stmt, 'i', $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    log_audit($myConnection, 'crowdfunding', $id, 'delete', $campaign['cfund_name'], $_SESSION['user_id']);
    $_SESSION['flash_message'] = 'Campaign deleted';
    header('Location: crowdfunding.php');
    exit;
}
?>
...
<html>
<head>
<title>AmigaSource.com - Delete Crowdfunding Campaign</title>
				...
				<table width="100%" cellpadding="1" cellspacing="1" class="bg-white" bgcolor="<?php echo bg_hex('white'); ?>">
					<tr>
						<td align="center" class="bg-red" bgcolor="<?php echo bg_hex('red'); ?>">
							<font class="txt-4-white" face="Verdana, sans-serif" size="4" color="<?php echo txt_hex('white'); ?>"><b>DELETE CROWDFUNDING CAMPAIGN</b></font>
						</td>
					</tr>
					<tr>
						<td class="bg-whitesmoke" bgcolor="<?php echo bg_hex('whitesmoke'); ?>" style="padding:16px;">
							<font class="txt-2-black" face="Verdana, sans-serif" size="2" color="<?php echo txt_hex('black'); ?>">
								Are you sure you want to delete <b><?php echo htmlspecialchars($campaign['cfund_name']); ?></b>?
							</font>
							<br><br>
							<center>
								<form method="post" action="crowdfunding_delete.php" style="display:inline;">
