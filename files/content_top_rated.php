<table width=100% align=center cellpadding=0>
	<tr>
		<td>
			<title>AmigaSource.com - TOP RATED</title>
...
$from = "t_links JOIN t_link_votes ON t_link_votes.link_id = t_links.id WHERE t_links.links_active=1";

$result_count = mysqli_query($myConnection, "SELECT COUNT(DISTINCT t_links.id) As total_records FROM $from");
...
$stmt = mysqli_prepare(
	$myConnection,
	"SELECT t_links.*, COUNT(t_link_votes.link_id) AS vote_count FROM $from GROUP BY t_links.id ORDER BY vote_count DESC, t_links.id DESC LIMIT ?, ?"
);
