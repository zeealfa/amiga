...
| login_db.php | Delegates to `includes/db.php` (Phase 02a), which opens the one shared `$myConnection` (mysqli) used by every include below |
...
| content_categories.php | Reads `t_cat_sub` by `$_GET['cat_id']` — now a mysqli prepared statement (Phase 02b, was unsanitized, see FINDINGS.md) |
| content_search.php | Renders the search form/results shell for content_search_proc.php |
| content_search_proc.php | POST handler: builds a `LIKE` query against `t_links` from `$_POST['search']` — now a mysqli prepared statement (Phase 02b, was unsanitized, see FINDINGS.md) |
| table_link.php | Row renderer for a single `t_links` result. Previously linked out to an external "Maestro" admin tool at testamigasource.com/ata/maestrotest/ — that link was stripped in Phase 02e (broken/dead path); real admin edit functionality is rebuilt in Phase 03/04 |
...
| File | Role |
|---|---|
| conn.php | Delegates to `includes/db.php` (Phase 02a) — previously its own separate connection with different credentials than login_db.php, see FINDINGS.md |
...
`table_link.php` used to link to `http://testamigasource.com/ata/maestrotest/t_links.php` — a
third-party admin tool ("Maestro") the client used for link management. That link was removed
from the public page in Phase 02e (it pointed at a broken/dead path). Phase 03 in roadmap.html
still covers building the proper replacement admin edit functionality.

## Shared includes (added Phase 02a)

| File | Role |
|---|---|
| includes/config.php | DB credential constants (`DB_HOST`/`DB_USER`/`DB_PASS`/`DB_NAME`) and pagination-size constants (`LINKS_PER_PAGE`, `NEWS_PER_PAGE`, added Phase 02e) |
| includes/db.php | Opens `$myConnection` from the config constants; required by both `login_db.php` and `ata/conn.php` |
| includes/functions.php | `render_pagination_menu()` (added Phase 02c), shared by `content_news.php` and `table_result_cat.php` |

## Stylesheet (added Phase 02d)

`style.css` holds the `.bg-<hexcolor>` / `.txt-<size>-<hexcolor|inherit>` classes extracted
from inline `bgcolor=""` / `<font face="" size="" color="">` markup across the site. Linked
from `page_builder.php` (the one include point common to every headless page fragment) and
directly from the two `files/ata/` pages that have their own `<head>`/no wrapper.
